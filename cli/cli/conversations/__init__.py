...
def _list(
        ctx: typer.Context,
        sender_id: Annotated[str, typer.Argument(help="The id of the sender to get all the conversations from.")],
):
    """
...
def download(
        ctx: typer.Context,
        ids: Annotated[List[str], typer.Argument(help="A list of the ids of your conversations.")],
    ...
    """
    print("Downloading...")
    r = ctx.parent.obj["r"].post(f"broker/conversations/{','.join(ids)}/download/", json=False)
