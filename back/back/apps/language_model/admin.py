...
from .forms import DataSourceForm, PromptConfigForm
...
def run_index_task(modeladmin, request, queryset):
    for rag_config in queryset:
        ...
        modeladmin.message_user(
            request, f"Index task started for {rag_config.name}", messages.SUCCESS
        )
...
def run_rag_deploy_task(modeladmin, request, queryset):
    for rag_config in queryset:
        rag_config.trigger_deploy()
        modeladmin.message_user(
            request, f"Deploy task started for {rag_config.name}", messages.SUCCESS
        )


run_rag_deploy_task.short_description = "Deploy selected RAG configs"
...
class RagConfigAdmin(admin.ModelAdmin):
    ...
    actions = [run_index_task, run_rag_deploy_task, run_test_task]
