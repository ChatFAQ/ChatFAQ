...
class KnowledgeBaseAPIViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=True, url_name='list-intents', url_path='list-intents', methods=['POST'])
    def list_intents(self, request, *args, **kwargs):
        ...
        if existing and suggested or not existing and not suggested:
            intents = Intent.objects.filter(knowledge_base=kb)
        elif existing:
            intents = Intent.objects.filter(knowledge_base=kb, suggested_intent=False)
        else:
            intents = Intent.objects.filter(knowledge_base=kb, suggested_intent=True)
...
class KnowledgeItemAPIViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=True, url_name='list-titles', url_path='list-titles', methods=['GET'])
    def list_auto_gen_titles(self, request, *args, **kwargs):
        """
        A view to list all the auto generated titles for a knowledge item:
        """
        titles = AutoGeneratedTitle.objects.filter(knowledge_item__id=kwargs["pk"]).all()
