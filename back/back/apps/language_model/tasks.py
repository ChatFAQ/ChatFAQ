...
def generate_titles(knowledge_base_pk, n_titles=10):
    ...
    ki = KnowledgeItem.objects.filter(knowledge_base=knowledge_base_pk)

    logger.info(f"Generating titles for {ki.count()} knowledge items")
    for item in tqdm(ki):
    ...
    logger.info(f"Titles generated for knowledge base: {ki[0].knowledge_base.name if ki.first() else 0}")
...
def generate_suggested_intents_task(knowledge_base_pk, _generate_titles=False):
    ...
    if _generate_titles:
        generate_titles(knowledge_base_pk)
