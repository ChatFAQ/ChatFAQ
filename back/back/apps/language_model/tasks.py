...
def get_similarity_scores(titles, retriever):
    ...
    """
    results = retriever.retrieve(titles)
...
def generate_suggested_intents_task(knowledge_base_pk):
    ...
    rag_conf = RAGConfig.objects.get(knowledge_base=knowledge_base_pk)
    ...
    del retriever, results_in_domain, results_out_domain, sim_in_domain, sim_out_domain, titles_in_domain # free memory
    ...
    messages = messages.filter(
        max_similarity__gte=new_intents_thresholds['max'],
        max_similarity__lte=new_intents_thresholds['min']
    )
