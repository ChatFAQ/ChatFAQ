...
from chat_rag.llms import GGMLModel, HFModel, OpenAIChatModel, VLLModel
...
@app.task()
def generate_new_intents_task(knowledge_base_pk):
    """
    ...
    messages_kb = MessageKnowledgeItem.objects.filter(
        knowledge_item__knowledge_base=knowledge_base_pk
    )

    results = MessageKnowledgeItem.objects.values("message_id").annotate(
    ...
    # now I want the message from the Message model with the max similarity

    logger.info(f"Number of messages: {messages_kb.count()}")
    logger.info(f"Number of messages with knowledge items: {results.count()}")

    # get max 100 auto generated titles
    auto_titles = AutoGeneratedTitle.objects.filter(
        knowledge_item__knowledge_base=knowledge_base_pk
    )[:100]
